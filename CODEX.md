...
- 패키지: `ai.devpath.platform` / 메인: `PlatformApplication`
- DB 연동 시 JPA 슬라이스/통합 테스트로 검증한다.
...
| mentor | 일반 계정과 분리된 멘토 접근 상태·초대 코드·admin 일일 초대 배치 |
| support | Turnstile·Redis 제한을 적용한 공개 문의 접수와 admin 문의 처리 |
